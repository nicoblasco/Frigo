...
{
    public partial class frmArticulosActualizarListaPrecio2 : Form
    {
        public frmArticulosActualizarListaPrecio2()
        {
            InitializeComponent();
        }
            ...
            if (result == System.Windows.Forms.DialogResult.Yes)
            {
                ...
                try
                {
                    foreach (DataGridViewRow row in gridArticulos.Rows)
                    {
                        ...
                        objManejaArticulos.ModificaPrecioL2Masivo (Convert.ToString(row.Cells[0].Value), Convert.ToDecimal(row.Cells[1].Value));
